...
class MultiSiteStudyFinderTests {
    ...
    private static Stream<Arguments> sampleStudiesWithExpectedClusterNumber() {
        return Stream.of(
                Arguments.of(
                        List.of(List.of(new Identifier().setSystem("drks.de").setValue("A")),
                                List.of(new Identifier().setSystem("drks.de").setValue("B")),
                                List.of(new Identifier().setSystem("drks.de").setValue("C")),
                                List.of(new Identifier().setSystem("drks.de").setValue("D"))), 4),
                Arguments.of(
                        List.of(List.of(new Identifier().setSystem("drks.de").setValue("A")),
                                ...
                                List.of(new Identifier().setSystem("drks.de").setValue("B")),
                                List.of(new Identifier().setSystem("drks.de").setValue("C"))), 3),
                Arguments.of(
                        List.of(List.of(new Identifier().setSystem("drks.de").setValue("A")),
                                List.of(new Identifier().setSystem("drks.de").setValue("A"),
                                        new Identifier().setSystem("clinicaltrials.gov").setValue("1")),
                                ...
                                List.of(new Identifier().setSystem("eudract").setValue("011"),
                                        new Identifier().setSystem("drks.de").setValue("A"))), 1)
        );
    }
    ...
    @Test
    void findMultiSiteStudies_whenGivenTwoStudiesWithSameIdentifierAndOneWithout_shouldReturnTwoClusters() {
        var input = List.of(
                new ResearchStudy()
                ...
                new ResearchStudy()
                        ...
                        .addIdentifier(new Identifier().setSystem("drks.de").setValue("A")),
                new ResearchStudy()
                        .addIdentifier(new Identifier().setSystem("drks.de").setValue("B"))
        );

        var result = sut.findMultiSiteStudies(input);

        assertThat(result).hasSize(2);
        assertThat(result).anySatisfy(set ->
                assertThat(set).allMatch(study -> study.getIdentifierFirstRep().getSystem().equals("drks.de") &&
    ...
    @Test
    void findMultiSiteStudies_whenGivenStudiesWithTransitiveConnectivity_shouldReturnClusterIncludingAll() {
        var input = List.of(
                new ResearchStudy()
                        .addIdentifier(new Identifier().setSystem("drks.de").setValue("A"))
        ...
        assertThat(result).anySatisfy(set ->
                assertThat(set)
                        .anyMatch(study -> study.getIdentifierFirstRep().getSystem().equals("drks.de") &&
                                study.getIdentifierFirstRep().getValue().equals("A")));
        assertThat(result).anySatisfy(set ->
                assertThat(set)
                        .anyMatch(study -> study.getIdentifierFirstRep().getSystem().equals("clinicaltrials.gov") &&
    ...
    void findMultiSiteStudies_whenGivenStudies_shouldReturnExpectedNumberOfCluster(
            List<List<Identifier>> studyIdentifiers,
            int expectedClusterSize) {
        var input = studyIdentifiers
                .stream()
