...
class MultiSiteStudyMergerTests {
    ...
    void mergeToMasterStudy_withSingleStudy_preservesContentsAndSetsMetaTagToMaster() {
        ...
        assertThat(result.getMeta().getTag()).anyMatch(tag ->
    ...
    void mergeToMasterStudy_withGivenStudies_setsRecruitmentStatusPerSite() {
        var input = List.of(new ResearchStudy().setStatus(ResearchStudy.ResearchStudyStatus.ACTIVE)
        ...
        assertThat(result.getSite()).allMatch(site -> site.hasExtension(fhirSystems.getMultiSiteStatusExtension()));

        var site1 = result.getSite().stream()
                .filter(site -> site.getReference().equals("Location/1"))
                .findFirst()
                .orElseThrow();
        var site1Status =
                ((CodeableConcept) site1.getExtensionByUrl(fhirSystems.getMultiSiteStatusExtension()).getValue())
                        .getCodingFirstRep()
                        .getCode();
        assertThat(site1Status).isEqualTo(ResearchStudy.ResearchStudyStatus.ACTIVE.toCode());

        var site2 = result.getSite().stream()
                .filter(site -> site.getReference().equals("Location/2"))
                .findFirst()
                .orElseThrow();
        var site2Status =
                ((CodeableConcept) site2.getExtensionByUrl(fhirSystems.getMultiSiteStatusExtension()).getValue())
                        .getCodingFirstRep()
                        .getCode();
    ...
    void mergeToMasterStudy_withGivenStudies_setsContactPerSite() {
        var input = List.of(new ResearchStudy().setContact(List.of(new ContactDetail().setName("A")))
                        .setSite(List.of(new Reference("Location/1"))),
                new ResearchStudy().setContact(List.of(new ContactDetail().setName("B")))
                        .setSite(List.of(new Reference("Location/2"))));

        var result = sut.mergeToMasterStudy(input);

        assertThat(result.getSite()).hasSize(2);
        assertThat(result.getSite()).allMatch(site -> site.hasExtension(fhirSystems.getMultiSiteContactExtension()));
        ...
        var site1 = result.getSite().stream()
                ...
                .orElseThrow();
        var site1Contact =
                ((ContactDetail) site1.getExtensionByUrl(fhirSystems.getMultiSiteContactExtension()).getValue());
        ...
        var site2 = result.getSite().stream()
                ...
                .orElseThrow();
        var site2Contact =
                ((ContactDetail) site2.getExtensionByUrl(fhirSystems.getMultiSiteContactExtension()).getValue());
        assertThat(site2Contact.getName()).isEqualTo("B");
    }

    @Test
    void mergeToMasterStudy_withAtLeatOneStudyActive_setsStatusToActive() {
        var input = List.of(new ResearchStudy().setStatus(ResearchStudy.ResearchStudyStatus.COMPLETED),
    ...
    void mergeToMasterStudy_withOnlyTranslatedTitle_setsTitleToTranslation() {
        var langExt = new Extension()
                .setUrl("lang")
                .setValue(new StringType("en"));
        var titleTransExt = new Extension()
                .setUrl("content")
                .setValue(new StringType("Title-En"));
        var titleExtension = new Extension()
    ...
    void mergeToMasterStudy_withTitleAlreadySet_doesNotOverrideWithTranslation() {
        ...
        var input = List.of(new ResearchStudy().setTitle("Original Title"),
