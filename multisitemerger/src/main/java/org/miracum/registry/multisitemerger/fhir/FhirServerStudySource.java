...
public class FhirServerStudySource {
    ...
    @Autowired
    public FhirServerStudySource(IGenericClient fhirClient, RetryTemplate retryTemplate, FhirSystems fhirSystems) {
        ...
        retryTemplate.registerListener(new RetryListenerSupport() {
            ...
            public <T, E extends Throwable> void onError(RetryContext context,
                                                         ...
                                                         Throwable throwable) {
                log.warn("Trying to retrieve studies from FHIR server caused error {}. {}. attempt.",
    ...
    public List<ResearchStudy> getStudies() throws Exception {
        ...
        var searchUrl = String.format("ResearchStudy?_tag:not=%s|master", fhirSystems.getStudyRoleTag());

        // retrieve all non-master studies from the server
        var results = retryTemplate.execute((RetryCallback<Bundle, Exception>) context -> fhirClient
                .search()
                .byUrl(searchUrl)
        ...
        do {
            ...
            if (results.getLink(Bundle.LINK_NEXT) != null) {
                ...
                results = retryTemplate.execute((RetryCallback<Bundle, Exception>) context ->
                        fhirClient.loadPage().next(resultsFinal).execute());
            } else {
                results = null;
            }
