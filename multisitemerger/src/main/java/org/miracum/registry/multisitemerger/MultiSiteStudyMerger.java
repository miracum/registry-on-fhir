...
public class MultiSiteStudyMerger {
    ...
    /**
     * Given a set of site-specific studies - all of which represent the same multi-centric study - returns a new
     * ResearchStudy resource where per-site information is merged.
     *
     * @param studies set of site-specific ResearchStudy resources belonging to the same multi-centric study
     * @return a single study record encompassing all site-specific information (such as recruitment status, contact
     *         points, etc.)
     */
    public ResearchStudy mergeToMasterStudy(List<ResearchStudy> studies) {
        ...
        masterStudy.getMeta().addTag()
                .setSystem(fhirSystems.getStudyRoleTag())
                .setCode("master");
        ...
        for (var study : studies) {
            if (!masterStudy.hasTitle()) {
                ...
                } else if (study.getTitleElement()
                        .hasExtension("http://hl7.org/fhir/StructureDefinition/translation")) {

                    var translation = study.getTitleElement()
            ...
            if (!masterStudy.hasDescription()) {
                ...
                } else if (study.getDescriptionElement()
                        .hasExtension("http://hl7.org/fhir/StructureDefinition/translation")) {

                    var translation = study.getDescriptionElement()
                            .getExtensionByUrl("http://hl7.org/fhir/StructureDefinition/translation");
            ...
            // put all identifiers in the hashmap in order to only end up with distinct identifiers in the end
            for (var identifier : study.getIdentifier()) {
                var id = String.format("%s|%s",
                        identifier.getSystem().toLowerCase().trim(),
                        identifier.getValue().toLowerCase().trim());

                uniqueIdentifiers.putIfAbsent(
                        id,
                        identifier);
            }
            ...
            if (study.hasSite()) {
                ...
                if (study.hasStatus()) {
                    ...
                    statusConcept.addCoding(new Coding()
                            .setSystem(studyStatus.getSystem())
                            .setCode(studyStatus.toCode()));
                    studySite.addExtension(fhirSystems.getMultiSiteStatusExtension(), statusConcept);
                }

                // add each contact as a new extension instance. This is necessary as the value of an extension
                // can not be a list (see https://www.hl7.org/fhir/extensibility.html).
                for (var contact : study.getContact()) {
                    studySite.addExtension(fhirSystems.getMultiSiteContactExtension(),
                            contact);
                }
    ...
    private void incrementMissingItemCount(String itemName) {
        missingItemMetrics.putIfAbsent(itemName,
                Metrics.globalRegistry.counter(MISSING_ITEM_METRIC_NAME, "item", itemName));
