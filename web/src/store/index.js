...
export const actions = {
  async fetchStudies() {
    ...
    const bundlePromise = await client.request(
      `ResearchStudy/?_include=ResearchStudy:site&_count=250&_pretty=false&_tag=${masterTag}&status=active`,
      {
        pageLimit: 0,
      }
    );
  ...
  async getStudyCount() {
    ...
    const request = `ResearchStudy/?_summary=count&status=active&_pretty=false&_tag=${masterTag}&status=active`;
